...
<template>
  <div class="menu-item" :class="item.selected ? 'selected' : ''" @mouseover="active = true" @mouseleave="active = false"
    @click.stop="routeToChild">
    <Suspense>
      <icon :fileName="item.img" :active="active" :selected="item.selected" :id="item.id" :size="'sm'" />
    </Suspense>
